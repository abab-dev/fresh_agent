"""
Simplified ripgrep-based text search.

Based on RepOMind's grep_string - minimal parameters, maximum clarity.
"""
...
class RipgrepTool(BaseTool):
    ...
    async def _search_ripgrep(self, query: str, path: str, context_lines: int):
        cmd = ["rg", "--json", "--no-heading", "-C", str(context_lines), "--max-count", "100"]
