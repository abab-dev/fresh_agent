...
class SubagentRunner:
    ...
    async def run(
        self, agent_name: str, task: str, context: Optional[str] = None, ui=None
    ) -> str:
        """
        ...
        for iteration in range(self.max_iterations):
            try:
                ...
                # Note: content can be None when LLM only returns tool calls
                assistant_msg = {
                    "role": "assistant",
                    "content": message.content or "",  # Prevent empty content error
                }
                ...
                for tool_call in tool_calls:
                    ...
                    if tool_name not in config.tools:
                        result = (
                            f"Error: Tool '{tool_name}' not allowed for this sub-agent"
                        )
                        success = False
                    else:
                        try:
                    ...
                    if ui:
                        if hasattr(ui, "subagent_tool_result"):
                            ui.subagent_tool_result(
                                agent_name, tool_name, success, str(result)
                            )
                        else:
    ...
    async def run_explorer(
        self, task: str, context: Optional[str] = None, ui=None
    ) -> str:
