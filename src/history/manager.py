...
import copy
import os
from typing import TYPE_CHECKING, Dict, Optional, List, Any, Protocol
...
class HistoryManager(BaseHistoryManager):
    ...
    def set_api_client(self, api_client: "LLMClient") -> None:
        self._api_client = api_client
    ...
    def update_token_usage(self, token_usage) -> None:
        ...
        if len(self.history_token_usage) == 0:
            self.history_token_usage.append(usage)
        else:
            self.history_token_usage[-1] = usage
    ...
    def start_new_chat(self, metadata: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.history_token_usage.append(TokenUsage(input_tokens=0, output_tokens=0, total_tokens=0))

    def finish_chat_get_response(self) -> str:
        assert len(self.messages_history) >= 2, "there must be more than or equal to 2 messages in history"
        task_messages = self.messages_history.pop()
        self.history_token_usage.pop()
        response = task_messages[-1]["content"] if task_messages else ""
        return response
        ...
        for msg in messages:
            ...
            if role == Role.TOOL:
                if content != "[Tool result cleared to save context]":
                    ...
                    formatted_lines.append(f"Tool({tool_name}): {content_str}...")
            else:
...
Provide focused summary (200-400 tokens):"""
        ...
        try:
            ...
            return "[Previous conversation compressed (summarization failed)]"
